...
const API_ENDPOINTS = [
  ...
  { name: 'RetroArch', url: 'https://lobby.libretro.com/list', type: 'json' },
...
async function fetchGlobalLobbies() {
  const results = [];

  for (const api of API_ENDPOINTS) {
    try {
      // Trim URL just in case, then encode
      const cleanUrl = api.url.trim();
      const proxyUrl = CORS_PROXY + encodeURIComponent(cleanUrl);

      const response = await fetch(proxyUrl, {
        method: 'GET',
        headers: { 
          'Accept': api.type === 'xml' ? 'text/xml' : 'application/json',
          'Origin': '*' // Help some proxies
        }
      });
