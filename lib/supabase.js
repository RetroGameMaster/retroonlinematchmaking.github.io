// lib/supabase.js - COMPLETE FIXED VERSION WITH ALL EXPORTS
...
async function updateAuthUI() {
    try {
        ...
        if (user) {
            ...
            // Determine what to show (Username preferred, fallback to email prefix)
            const displayName = profile?.username || user.email?.split('@')[0];
            const displayAvatar = profile?.avatar_url || `https://ui-avatars.com/api/?name=${displayName}&background=06b6d4&color=fff`;

            console.log('User logged in:', displayName);
            
            authButtons.innerHTML = `
                <div class="flex items-center space-x-4">
                    <a href="#/profile/${displayName}" class="flex items-center gap-2 group">
                        <img src="${displayAvatar}" alt="${displayName}" class="w-8 h-8 rounded-full border border-gray-600 group-hover:border-cyan-400 transition">
                        <span class="text-gray-300 font-bold group-hover:text-cyan-400 transition">${displayName}</span>
                    </a>
                    <button id="logout-btn" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded transition text-sm">
                        Logout
                    </button>
                </div>
            `;
...
// NEW FUNCTION: Upload game image
async function uploadGameImage(file, gameId, type = 'cover') {
    try {
        ...
        const fileExt = file.name.split('.').pop();
        const folder = type === 'cover' ? 'covers' : 'screenshots';
        const fileName = `${folder}/${gameId}/${Date.now()}.${fileExt}`;
...
// NEW FUNCTION: Upload submission images
async function uploadSubmissionImages(submissionId, coverFile, screenshotFiles) {
    try {
        const uploadedUrls = {
            cover: null,
            screenshots: []
        ...
        if (coverFile) {
            const coverResult = await uploadGameImage(coverFile, submissionId, 'cover');
            if (coverResult.success) {
                uploadedUrls.cover = coverResult.url;
            }
        }
        
        // Upload screenshots if provided
        if (screenshotFiles && screenshotFiles.length > 0) {
            for (const file of screenshotFiles.slice(0, 3)) { // Max 3 screenshots
                const screenshotResult = await uploadGameImage(file, submissionId, 'screenshots');
                if (screenshotResult.success) {
                    uploadedUrls.screenshots.push(screenshotResult.url);
                }
            }
        }
        ...
        console.error('Error uploading submission images:', error);
        return { success: false, error: error.message };
    }
}

// NEW FUNCTION: Get game by ID - ADD THIS
...
// NEW FUNCTION: Check if user can edit a game - ADD THIS
async function canEditGame(gameId) {
    try {
        const user = await getCurrentUser();
        if (!user) return false;
        ...
        // Check if user submitted this game
        const { data: game, error } = await supabase
            .from('games')
            .select('submitted_by')
            .eq('id', gameId)
        ...
        return game.submitted_by === user.id;
    } catch (error) {
...
// Export all functions - UPDATED WITH NEW EXPORTS
export {
    supabase,
    supabaseUrl,
    supabaseAnonKey,
...
// Initialize the global rom object if it doesn't exist
if (!window.rom) {
    window.rom = {
        supabase: supabase,
        loadModule: null, // Will be set by app.js
        currentUser: null
    };
} else {
    // Make sure rom has supabase
    window.rom.supabase = supabase;
}

console.log('✅ Supabase attached to window and rom');
