...
function initSubmitGame(rom) {
  ...
  document.getElementById('gameSubmissionForm').addEventListener('submit', async function(e) {
    ...
    const gameData = {
      ...
      console: consoleSelect.value, // ✅ FIXED: Use single console value
    ...
    try {
      // Save the game submission WITH SLUG
  ...
  async function saveGameSubmission(gameData, supabaseClient) {
    // ✅ FIXED: Use `console` instead of `platforms`
    const submissionData = {
      title: gameData.title,
      console: gameData.console, // ← CORRECT FIELD
