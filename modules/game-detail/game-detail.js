...
// ===== RENDER GAME FUNCTION (FIXED: Added 'async') =====
async function renderGame(game, container, rom) {
    ...
    // 3. Check for Guides (Async Operation)
    let guideButtonHTML = '';
    try {
        const { data: guides, error } = await rom.supabase
            .from('guides')
        ...
        if (!error && guides && guides.length > 0) {
            ...
            guideButtonHTML = `
                <a href="${link}" class="block w-full text-center py-3 mt-4 bg-purple-900/50 hover:bg-purple-800 border border-purple-500 text-purple-200 font-bold rounded-lg transition shadow-[0_0_15px_rgba(168,85,247,0.4)]">
                    📖 Read Setup Guide & Wiki
                </a>
            `;
        }
    } catch (e) {
        console.warn('Guide check skipped or failed:', e.message);
    }

    // 4. Prepare Rating Section HTML
    const ratingHTML = `
    ...
    // 5. Prepare Button HTML (Playing List)
    ...
    // 6. Prepare Metadata Panel (Semantic DL for SEO)
